...
在状态模式中,类的行为是基于它的状态改变的,允许对象在内部状态发生改变时改变它的行为,对象看起来
好像修改了它的类,主要解决对象的行为依赖它的状态,并且可以根据它的状态而改变它的相关行为.

- 生活例子

零食自动贩卖机,会有不同的状态,根据我们放入钱币的数量作出不同的反应,根据我们的选择会执行如下操作:
拒绝我们,因为货物售罄;拒绝我们,因为钱币不足;出货,因为钱币整好;
...
定义一系列的算法,把它们一个个封装起来, 并且使它们可相互替换.策略模式鼓励使用多种算法解决一个
问题,

- 生活例子

赶飞机,可以打车去,可以开车去,可以地铁去.

- 软件例子

python的sorted和list.sort()